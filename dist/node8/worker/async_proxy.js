  ...
  const handleMessage = workerId => event => {
    ...
    switch (msg.event) {
      case 'method-return':
        if (msg.err) {
          // fail
          const err = new Error(msg.err); // add worker stack
...
AsyncProxy.prototype.seedRandom = async function (workerId, size) {
  const buf = await _crypto.default.random.getRandomBytes(size);
  this.workers[workerId].postMessage({
    event: 'seed-random',
    buf
  }, _util.default.getTransferables(buf));
};
/**
...
AsyncProxy.prototype.delegate = function (method, options) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    this.workers[workerId].postMessage({
      id: id,
      event: method,
      options: _packet.default.clone.clonePackets(options)
    }, _util.default.getTransferables(options));
    this.workers[workerId].requests++; // remember to handle parsing cloned packets from worker

    this.tasks[id] = {
      resolve: data => resolve(_packet.default.clone.parseClonedPackets(_util.default.restoreStreams(data), method)),
      reject
    };
  });
};
