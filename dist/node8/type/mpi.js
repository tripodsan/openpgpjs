...
function MPI(data) {
  ...
  if (data instanceof MPI) {
    this.data = data.data;
  } else if (_bn.default.isBN(data)) {
    this.fromBN(data);
  } else if (_util.default.isUint8Array(data)) {
    this.fromUint8Array(data);
  } else if (_util.default.isString(data)) {
...
MPI.prototype.read = function (bytes, endian = 'be') {
  if (_util.default.isString(bytes)) {
    bytes = _util.default.str_to_Uint8Array(bytes);
  }
...
MPI.prototype.write = function (endian, length) {
  return _util.default.Uint8Array_to_MPI(this.toUint8Array(endian, length));
};

MPI.prototype.bitLength = function () {
  return (this.data.length - 1) * 8 + _util.default.nbits(this.data[0]);
};
...
MPI.prototype.toString = function () {
  return _util.default.Uint8Array_to_str(this.toUint8Array());
};

MPI.prototype.fromString = function (str, endian = 'be') {
  this.fromUint8Array(_util.default.str_to_Uint8Array(str), endian);
};

MPI.prototype.toBN = function () {
  return new _bn.default(this.toUint8Array());
};
