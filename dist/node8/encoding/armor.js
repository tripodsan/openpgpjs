...
function addheader(customComment) {
  ...
  if (_config.default.show_version) {
    result += "Version: " + _config.default.versionstring + '\r\n';
  }

  if (_config.default.show_comment) {
    result += "Comment: " + _config.default.commentstring + '\r\n';
  }
...
function getCheckSum(data) {
  ...
  return _base.default.encode(crc);
}
...
function createcrc24(input) {
  let crc = 0xB704CE;
  return _webStreamTools.default.transform(input, value => {
...
function verifyHeaders(headers) {
  for (let i = 0; i < headers.length; i++) {
    ...
    if (!/^(Version|Comment|MessageID|Hash|Charset): .+$/.test(headers[i])) {
      _util.default.print_debug_error(new Error('Unknown header: ' + headers[i]));
    }
  }
}
/**
...
function dearmor(input) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let data = _base.default.decode(_webStreamTools.default.transformPair(input, async (readable, writable) => {
      ...
      data = _webStreamTools.default.transformPair(data, async (readable, writable) => {
        const checksumVerified = getCheckSum(_webStreamTools.default.passiveClone(readable));
        await _webStreamTools.default.pipe(readable, writable, {
          preventClose: true
        });
        ...
        try {
          const checksumVerifiedString = await _webStreamTools.default.readToEnd(checksumVerified);

          if (checksum !== checksumVerifiedString && (checksum || _config.default.checksum_required)) {
...
function armor(messagetype, body, partindex, parttotal, customComment) {
  ...
  if (messagetype === _enums.default.armor.signed) {
    text = body.text;
    hash = body.hash;
    body = body.data;
  }

  const bodyClone = _webStreamTools.default.passiveClone(body);

  const result = [];

  switch (messagetype) {
    case _enums.default.armor.multipart_section:
      result.push("-----BEGIN PGP MESSAGE, PART " + partindex + "/" + parttotal + "-----\r\n");
      result.push(addheader(customComment));
      result.push(_base.default.encode(body));
      result.push("\r\n=", getCheckSum(bodyClone), "\r\n");
      result.push("-----END PGP MESSAGE, PART " + partindex + "/" + parttotal + "-----\r\n");
      break;

    case _enums.default.armor.multipart_last:
    ...
    case _enums.default.armor.signed:
    ...
    case _enums.default.armor.message:
    ...
    case _enums.default.armor.public_key:
    ...
    case _enums.default.armor.private_key:
    ...
    case _enums.default.armor.signature:
  ...
  return _util.default.concat(result);
}
