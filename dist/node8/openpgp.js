...
function initWorker({
  ...
  if (workers.length || typeof window !== 'undefined' && window.Worker) {
    asyncProxy = new _async_proxy.default({
      path,
      n,
      workers,
...
function generateKey({
  ...
  if (_util.default.getWebCryptoAll() && numBits < 2048) {
    throw new Error('numBits should be 2048 or 4096, found: ' + numBits);
  }

  if (!_util.default.getWebCryptoAll() && asyncProxy) {
...
function encrypt({
  ...
  passwords,
  sessionKey,
  compression = _config.default.compression,
  armor = true,
...
function checkBinary(data, name) {
  if (!_util.default.isUint8Array(data)) {
...
function toArray(param) {
  if (param && !_util.default.isArray(param)) {
    param = [param];
  }
...
async function convertStream(data, streaming) {
  if (!streaming && _util.default.isStream(data)) {
    return _webStreamTools.default.readToEnd(data);
  }
  ...
  if (streaming === 'node') {
    data = _webStreamTools.default.webToNode(data);
  }
...
async function convertStreams(obj, streaming, keys = []) {
  if (Object.prototype.isPrototypeOf(obj) && !Uint8Array.prototype.isPrototypeOf(obj)) {
    await Promise.all(Object.entries(obj).map(async ([key, value]) => {
      ...
      if (_util.default.isStream(value) || keys.includes(key)) {
...
function linkStreams(result, message, erroringStream) {
  result.data = _webStreamTools.default.transformPair(message.packets.stream, async (readable, writable) => {
    await _webStreamTools.default.pipe(result.data, writable, {
      preventClose: true
    });
    ...
    try {
      ...
      await _webStreamTools.default.readToEnd(erroringStream || readable, arr => arr);
      await writer.close();
    } catch (e) {
...
function onError(message, error) {
  // log the stack trace
  _util.default.print_debug_error(error); // update error message
...
function nativeAEAD() {
  return _config.default.aead_protect && ((_config.default.aead_protect_version !== 4 || _config.default.aead_mode === _enums.default.aead.experimental_gcm) && _util.default.getWebCrypto() || _config.default.aead_protect_version === 4 && _config.default.aead_mode === _enums.default.aead.eax && _util.default.getWebCrypto());
}