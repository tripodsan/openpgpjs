...
var _default = {
  readSimpleLength: function (bytes) {
    ...
    if (type < 192) {
      ...
      len = _util.default.readNumber(bytes.subarray(1, 1 + 4));
      offset = 5;
    }
  ...
  writeSimpleLength: function (length) {
    ...
    return _util.default.concatUint8Array([new Uint8Array([255]), _util.default.writeNumber(length, 4)]);
  },
  ...
  writeHeader: function (tag_type, length) {
    /* we're only generating v4 packet headers here */
    return _util.default.concatUint8Array([this.writeTag(tag_type), this.writeSimpleLength(length)]);
  },
  ...
  writeOldHeader: function (tag_type, length) {
    if (length < 256) {
      ...
      return _util.default.concatUint8Array([new Uint8Array([0x80 | tag_type << 2 | 1]), _util.default.writeNumber(length, 2)]);
    }

    return _util.default.concatUint8Array([new Uint8Array([0x80 | tag_type << 2 | 2]), _util.default.writeNumber(length, 4)]);
  },
  ...
  supportsStreaming: function (tag_type) {
    return [_enums.default.packet.literal, _enums.default.packet.compressed, _enums.default.packet.symmetricallyEncrypted, _enums.default.packet.symEncryptedIntegrityProtected, _enums.default.packet.symEncryptedAEADProtected].includes(tag_type);
  },
  ...
  read: async function (input, callback) {
    const reader = _webStreamTools.default.getReader(input);

    let writer;

    try {
      const peekedBytes = await reader.peekBytes(2); // some sanity checks
      ...
      if (streaming) {
        ...
        writer = _webStreamTools.default.getWriter(transform.writable);
      ...
      do {
        if (!format) {
          ...
          if (lengthByte < 192) {
            ...
            packet_length = (lengthByte - 192 << 8) + (await reader.readByte()) + 192; // 4.2.2.4. Partial Body Lengths
