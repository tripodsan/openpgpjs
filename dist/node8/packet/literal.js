...
function Literal(date = new Date()) {
  this.tag = _enums.default.packet.literal;
  this.format = 'utf8'; // default format for literal data packets

  this.date = _util.default.normalizeDate(date);
...
Literal.prototype.getText = function (clone = false) {
  if (this.text === null || _util.default.isStream(this.text)) {
    // Assume that this.text has been read
    this.text = _util.default.nativeEOL(_util.default.decode_utf8(this.getBytes(clone)));
  }
...
Literal.prototype.getBytes = function (clone = false) {
  if (this.data === null) {
    // normalize EOL to \r\n and encode UTF8
    this.data = _util.default.encode_utf8(_util.default.canonicalizeEOL(this.text));
  }

  if (clone) {
    return _webStreamTools.default.passiveClone(this.data);
  }
...
Literal.prototype.read = async function (bytes) {
  await _webStreamTools.default.parse(bytes, async reader => {
    ...
    const format = _enums.default.read(_enums.default.literal, (await reader.readByte()));

    const filename_len = await reader.readByte();
    this.filename = _util.default.decode_utf8((await reader.readBytes(filename_len)));
    this.date = _util.default.readDate((await reader.readBytes(4)));
...
Literal.prototype.write = function () {
  const filename = _util.default.encode_utf8(this.filename);

  const filename_length = new Uint8Array([filename.length]);
  const format = new Uint8Array([_enums.default.write(_enums.default.literal, this.format)]);

  const date = _util.default.writeDate(this.date);

  const data = this.getBytes();
  return _util.default.concat([format, filename_length, filename, date, data]);
};
