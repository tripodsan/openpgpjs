...
function SymEncryptedAEADProtected() {
  this.tag = _enums.default.packet.symEncryptedAEADProtected;
...
var _default = SymEncryptedAEADProtected;
/**
...
SymEncryptedAEADProtected.prototype.read = async function (bytes) {
  await _webStreamTools.default.parse(bytes, async reader => {
    ...
    if (_config.default.aead_protect_version === 4) {
      this.cipherAlgo = await reader.readByte();
      this.aeadAlgo = await reader.readByte();
      this.chunkSizeByte = await reader.readByte();
    } else {
      this.aeadAlgo = _enums.default.aead.experimental_gcm;
    }

    const mode = _crypto.default[_enums.default.read(_enums.default.aead, this.aeadAlgo)];
...
SymEncryptedAEADProtected.prototype.write = function () {
  if (_config.default.aead_protect_version === 4) {
    return _util.default.concat([new Uint8Array([this.version, this.cipherAlgo, this.aeadAlgo, this.chunkSizeByte]), this.iv, this.encrypted]);
  }

  return _util.default.concat([new Uint8Array([this.version]), this.iv, this.encrypted]);
};
/**
...
SymEncryptedAEADProtected.prototype.decrypt = async function (sessionKeyAlgorithm, key, streaming) {
  if (_config.default.aead_protect_version !== 4) {
    this.cipherAlgo = _enums.default.write(_enums.default.symmetric, sessionKeyAlgorithm);
  }

  await this.packets.read((await this.crypt('decrypt', key, _webStreamTools.default.clone(this.encrypted), streaming)));
  return true;
};
/**
...
SymEncryptedAEADProtected.prototype.encrypt = async function (sessionKeyAlgorithm, key, streaming) {
  this.cipherAlgo = _enums.default.write(_enums.default.symmetric, sessionKeyAlgorithm);
  this.aeadAlgo = _config.default.aead_protect_version === 4 ? _enums.default.write(_enums.default.aead, this.aeadAlgorithm) : _enums.default.aead.experimental_gcm;

  const mode = _crypto.default[_enums.default.read(_enums.default.aead, this.aeadAlgo)];

  this.iv = await _crypto.default.random.getRandomBytes(mode.ivLength); // generate new random IV

  this.chunkSizeByte = _config.default.aead_chunk_size_byte;
...
SymEncryptedAEADProtected.prototype.crypt = async function (fn, key, data, streaming) {
  const cipher = _enums.default.read(_enums.default.symmetric, this.cipherAlgo);
  ...
  if (_config.default.aead_protect_version === 4) {
    ...
    let queuedBytes = 0;
    const iv = this.iv;
    return _webStreamTools.default.transformPair(data, async (readable, writable) => {
      const reader = _webStreamTools.default.getReader(readable);

      const buffer = new TransformStream({}, {
        highWaterMark: streaming ? _util.default.getHardwareConcurrency() * 2 ** (_config.default.aead_chunk_size_byte + 6) : Infinity,
        size: array => array.length
      });

      _webStreamTools.default.pipe(buffer.readable, writable);

      const writer = _webStreamTools.default.getWriter(buffer.writable);
    ...
    return modeInstance[fn]((await _webStreamTools.default.readToEnd(data)), this.iv);
  }
};