...
var _keyid = _interopRequireDefault(require("../type/keyid.js"));

var _mpi = _interopRequireDefault(require("../type/mpi.js"));
...
function Signature(date = new Date()) {
  this.tag = _enums.default.packet.signature;
  this.version = 4;
  ...
  this.created = _util.default.normalizeDate(date);
...
Signature.prototype.read = function (bytes) {
  ...
  const subpackets = bytes => {
    ...
    const subpacket_length = _util.default.readNumber(bytes.subarray(0, 2));

    let i = 2; // subpacket data set (zero or more subpackets)

    while (i < 2 + subpacket_length) {
      const len = _packet.default.readSimpleLength(bytes.subarray(i, bytes.length));
  ...
  i += subpackets(bytes.subarray(i, bytes.length), true); // A V4 signature hashes the packet body
  ...
  const sigDataLength = i; // unhashed subpackets

  i += subpackets(bytes.subarray(i, bytes.length), false);
  this.unhashedSubpackets = bytes.subarray(sigDataLength, i); // Two-octet field holding left 16 bits of signed hash value.
...
Signature.prototype.write = function () {
  ...
  arr.push(this.unhashedSubpackets ? this.unhashedSubpackets : _util.default.writeNumber(0, 2));
  arr.push(this.signedHashValue);
  arr.push(_webStreamTools.default.clone(this.signature));
...
Signature.prototype.sign = async function (key, data) {
  const signatureType = _enums.default.write(_enums.default.signature, this.signatureType);

  const publicKeyAlgorithm = _enums.default.write(_enums.default.publicKey, this.publicKeyAlgorithm);

  const hashAlgorithm = _enums.default.write(_enums.default.hash, this.hashAlgorithm);
  ...
  this.signatureData = _util.default.concat(arr);
  const toHash = this.toHash(data);
  const hash = this.hash(data, toHash);
  this.signedHashValue = _webStreamTools.default.slice(_webStreamTools.default.clone(hash), 0, 2);
  const params = key.params;
  this.signature = _webStreamTools.default.fromAsync(async () => _crypto.default.signature.sign(publicKeyAlgorithm, hashAlgorithm, params, toHash, (await _webStreamTools.default.readToEnd(hash)))); // Store the fact that this signature is valid, e.g. for when we call `await
...
Signature.prototype.write_all_sub_packets = function () {
  const sub = _enums.default.signatureSubpacket;
  const arr = [];
  let bytes;

  if (this.created !== null) {
    arr.push(write_sub_packet(sub.signature_creation_time, _util.default.writeDate(this.created)));
  }

  if (this.signatureExpirationTime !== null) {
    arr.push(write_sub_packet(sub.signature_expiration_time, _util.default.writeNumber(this.signatureExpirationTime, 4)));
  }
  ...
  if (this.keyExpirationTime !== null) {
    arr.push(write_sub_packet(sub.key_expiration_time, _util.default.writeNumber(this.keyExpirationTime, 4)));
  }

  if (this.preferredSymmetricAlgorithms !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.preferredSymmetricAlgorithms));
  ...
  if (this.revocationKeyClass !== null) {
    ...
    bytes = _util.default.concat([bytes, this.revocationKeyFingerprint]);
  ...
  if (this.notation !== null) {
    Object.entries(this.notation).forEach(([name, value]) => {
      ...
      bytes.push(_util.default.writeNumber(name.length, 2)); // 2 octets of value length

      bytes.push(_util.default.writeNumber(value.length, 2));
      bytes.push(_util.default.str_to_Uint8Array(name + value));
      bytes = _util.default.concat(bytes);
      arr.push(write_sub_packet(sub.notation_data, bytes));
    });
  }

  if (this.preferredHashAlgorithms !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.preferredHashAlgorithms));
    arr.push(write_sub_packet(sub.preferred_hash_algorithms, bytes));
  }

  if (this.preferredCompressionAlgorithms !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.preferredCompressionAlgorithms));
    arr.push(write_sub_packet(sub.preferred_compression_algorithms, bytes));
  }

  if (this.keyServerPreferences !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.keyServerPreferences));
    arr.push(write_sub_packet(sub.key_server_preferences, bytes));
  }

  if (this.preferredKeyServer !== null) {
    arr.push(write_sub_packet(sub.preferred_key_server, _util.default.str_to_Uint8Array(this.preferredKeyServer)));
  }
  ...
  if (this.policyURI !== null) {
    arr.push(write_sub_packet(sub.policy_uri, _util.default.str_to_Uint8Array(this.policyURI)));
  }

  if (this.keyFlags !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.keyFlags));
    arr.push(write_sub_packet(sub.key_flags, bytes));
  }

  if (this.signersUserId !== null) {
    arr.push(write_sub_packet(sub.signers_user_id, _util.default.str_to_Uint8Array(this.signersUserId)));
  }

  if (this.reasonForRevocationFlag !== null) {
    bytes = _util.default.str_to_Uint8Array(String.fromCharCode(this.reasonForRevocationFlag) + this.reasonForRevocationString);
    arr.push(write_sub_packet(sub.reason_for_revocation, bytes));
  }

  if (this.features !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.features));
  ...
  if (this.signatureTargetPublicKeyAlgorithm !== null) {
    ...
    bytes.push(_util.default.str_to_Uint8Array(this.signatureTargetHash));
    bytes = _util.default.concat(bytes);
  ...
  if (this.preferredAeadAlgorithms !== null) {
    bytes = _util.default.str_to_Uint8Array(_util.default.Uint8Array_to_str(this.preferredAeadAlgorithms));
    arr.push(write_sub_packet(sub.preferred_aead_algorithms, bytes));
  }

  const result = _util.default.concat(arr);

  const length = _util.default.writeNumber(result.length, 2);

  return _util.default.concat([length, result]);
};
/**
...
function write_sub_packet(type, data) {
  const arr = [];
  arr.push(_packet.default.writeSimpleLength(data.length + 1));
...
Signature.prototype.read_sub_packet = function (bytes) {
  ...
  let seconds; // subpacket type

  switch (type) {
    case 2:
      // Signature Creation Time
      this.created = _util.default.readDate(bytes.subarray(mypos, bytes.length));
      break;

    case 3:
      // Signature Expiration Time in seconds
      seconds = _util.default.readNumber(bytes.subarray(mypos, bytes.length));
    ...
    case 20:
      ...
      if (bytes[mypos] === 0x80) {
        ...
        const m = _util.default.readNumber(bytes.subarray(mypos, mypos + 2));

        mypos += 2;

        const n = _util.default.readNumber(bytes.subarray(mypos, mypos + 2));

        mypos += 2;

        const name = _util.default.Uint8Array_to_str(bytes.subarray(mypos, mypos + m));

        const value = _util.default.Uint8Array_to_str(bytes.subarray(mypos + m, mypos + m + n));

        this.notation = this.notation || {};
        this.notation[name] = value;
      } else {
        _util.default.print_debug("Unsupported notation flag " + bytes[mypos]);
      }
    ...
    case 24:
      ...
      this.preferredKeyServer = _util.default.Uint8Array_to_str(bytes.subarray(mypos, bytes.length));
      break;
    ...
    case 26:
      // Policy URI
      this.policyURI = _util.default.Uint8Array_to_str(bytes.subarray(mypos, bytes.length));
      break;
    ...
    case 28:
      // Signer's User ID
      this.signersUserId = _util.default.Uint8Array_to_str(bytes.subarray(mypos, bytes.length));
      break;

    case 29:
      // Reason for Revocation
      this.reasonForRevocationFlag = bytes[mypos++];
      this.reasonForRevocationString = _util.default.Uint8Array_to_str(bytes.subarray(mypos, bytes.length));
      break;
    ...
    case 31:
      {
        ...
        const len = _crypto.default.getHashByteLength(this.signatureTargetHashAlgorithm);

        this.signatureTargetHash = _util.default.Uint8Array_to_str(bytes.subarray(mypos, mypos + len));
        break;
      }
    ...
    default:
      _util.default.print_debug("Unknown signature subpacket type " + type + " @:" + mypos);
...
Signature.prototype.toSign = function (type, data) {
  const t = _enums.default.signature;

  switch (type) {
    case t.binary:
      if (data.text !== null) {
        return _util.default.str_to_Uint8Array(data.getText(true));
      }
    ...
    case t.text:
      {
        let text = data.getText(true); // normalize EOL to \r\n

        text = _util.default.canonicalizeEOL(text); // encode UTF8

        return _util.default.encode_utf8(text);
      }
    ...
    case t.cert_revocation:
      {
        ...
        return _util.default.concat([this.toSign(t.key, data), new Uint8Array([tag]), _util.default.writeNumber(bytes.length, 4), bytes]);
      }

    case t.subkey_binding:
    case t.subkey_revocation:
    case t.key_binding:
      return _util.default.concat([this.toSign(t.key, data), this.toSign(t.key, {
        key: data.bind
      })]);
...
Signature.prototype.calculateTrailer = function () {
  let length = 0;
  return _webStreamTools.default.transform(_webStreamTools.default.clone(this.signatureData), value => {
    length += value.length;
  }, () => {
    const first = new Uint8Array([4, 0xFF]); //Version, ?

    return _util.default.concat([first, _util.default.writeNumber(length, 4)]);
  });
};
...
Signature.prototype.toHash = function (data) {
  ...
  return _util.default.concat([bytes, this.signatureData, this.calculateTrailer()]);
};
...
Signature.prototype.hash = function (data, toHash, streaming = true) {
  ...
  if (!streaming && _util.default.isStream(toHash)) {
    return _webStreamTools.default.fromAsync(async () => this.hash(data, (await _webStreamTools.default.readToEnd(toHash))));
  }

  return _crypto.default.hash.digest(hashAlgorithm, toHash);
};
/**
...
Signature.prototype.verify = async function (key, data) {
  ...
  hash = await _webStreamTools.default.readToEnd(hash);

  if (this.signedHashValue[0] !== hash[0] || this.signedHashValue[1] !== hash[1]) {
    this.verified = false;
  } else {
    let mpicount = 0; // Algorithm-Specific Fields for RSA signatures:
    ...
    if (publicKeyAlgorithm > 0 && publicKeyAlgorithm < 4) {
      ...
      //      - MPI of DSA value r.
      //      - MPI of DSA value s.
    } else if (publicKeyAlgorithm === _enums.default.publicKey.dsa || publicKeyAlgorithm === _enums.default.publicKey.ecdsa || publicKeyAlgorithm === _enums.default.publicKey.eddsa) {
      mpicount = 2;
    } // EdDSA signature parameters are encoded in little-endian format
    // https://tools.ietf.org/html/rfc8032#section-5.1.2


    const endian = publicKeyAlgorithm === _enums.default.publicKey.eddsa ? 'le' : 'be';
    const mpi = [];
    let i = 0;
    this.signature = await _webStreamTools.default.readToEnd(this.signature);

    for (let j = 0; j < mpicount; j++) {
      mpi[j] = new _mpi.default();
      i += mpi[j].read(this.signature.subarray(i, this.signature.length), endian);
    }

    this.verified = await _crypto.default.signature.verify(publicKeyAlgorithm, hashAlgorithm, mpi, key.params, toHash, hash);
  }
