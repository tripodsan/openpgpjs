...
function PublicKey(date = new Date()) {
   ...
   */
  this.tag = _enums.default.packet.publicKey;
  /**
   * Packet version
   * @type {Integer}
   */
...
PublicKey.prototype.read = function (bytes) {
  let pos = 0; // A one-octet version number (3, 4 or 5).

  this.version = bytes[pos++];

  if (this.version === 4 || this.version === 5) {
    // - A four-octet number denoting the time that the key was created.
    this.created = _util.default.readDate(bytes.subarray(pos, pos + 4));
    pos += 4; // - A one-octet number denoting the public-key algorithm of this key.

    this.algorithm = _enums.default.read(_enums.default.publicKey, bytes[pos++]);
    ...
    const types = _crypto.default.getPubKeyParamTypes(algo);

    this.params = _crypto.default.constructParams(types);
...
PublicKey.prototype.write = function () {
  ...
  arr.push(_util.default.writeDate(this.created)); // A one-octet number denoting the public-key algorithm of this key

  const algo = _enums.default.write(_enums.default.publicKey, this.algorithm);

  arr.push(new Uint8Array([algo]));

  const paramCount = _crypto.default.getPubKeyParamTypes(algo).length;

  const params = _util.default.concatUint8Array(this.params.slice(0, paramCount).map(param => param.write()));

  if (this.version === 5) {
    // A four-octet scalar octet count for the following key material
    arr.push(_util.default.writeNumber(params.length, 4));
  } // Algorithm-specific params
...
PublicKey.prototype.writeOld = function () {
  ...
  return _util.default.concatUint8Array([new Uint8Array([0x99]), _util.default.writeNumber(bytes.length, 2), bytes]);
};
/**
...
PublicKey.prototype.getKeyId = function () {
  ...
  this.keyid = new _keyid.default();

  if (this.version === 5) {
    this.keyid.read(_util.default.hex_to_Uint8Array(this.getFingerprint()).subarray(0, 8));
  } else if (this.version === 4) {
    this.keyid.read(_util.default.hex_to_Uint8Array(this.getFingerprint()).subarray(12, 20));
  }
...
PublicKey.prototype.getFingerprintBytes = function () {
  ...
  if (this.version === 5) {
    const bytes = this.writePublicKey();
    toHash = _util.default.concatUint8Array([new Uint8Array([0x9A]), _util.default.writeNumber(bytes.length, 4), bytes]);
    this.fingerprint = _crypto.default.hash.sha256(toHash);
  } else if (this.version === 4) {
    toHash = this.writeOld();
    this.fingerprint = _crypto.default.hash.sha1(toHash);
  }
...
PublicKey.prototype.getFingerprint = function () {
  return _util.default.Uint8Array_to_hex(this.getFingerprintBytes());
};
/**
...
PublicKey.prototype.getAlgorithmInfo = function () {
  ...
  if (this.params[0] instanceof _mpi.default) {
