...
function SecretKey(date = new Date()) {
  _public_key.default.call(this, date);
  /**
   * Packet type
  ...
  this.tag = _enums.default.packet.secretKey;
  /**
...
SecretKey.prototype = new _public_key.default();
...
function get_hash_fn(hash) {
  if (hash === 'sha1') {
    return _crypto.default.hash.sha1;
  }

  return function (c) {
    return _util.default.writeNumber(_util.default.calc_checksum(c), 2);
  };
} // Helper function
...
function parse_cleartext_params(hash_algorithm, cleartext, algorithm) {
  if (hash_algorithm) {
    ...
    const hashtext = _util.default.Uint8Array_to_str(cleartext.subarray(cleartext.length - hashlen, cleartext.length));

    cleartext = cleartext.subarray(0, cleartext.length - hashlen);

    const hash = _util.default.Uint8Array_to_str(hashfn(cleartext));
  ...
  const algo = _enums.default.write(_enums.default.publicKey, algorithm);

  const types = _crypto.default.getPrivKeyParamTypes(algo);

  const params = _crypto.default.constructParams(types);
...
function write_cleartext_params(hash_algorithm, algorithm, params) {
  ...
  const numPublicParams = _crypto.default.getPubKeyParamTypes(algo).length;

  for (let i = numPublicParams; i < params.length; i++) {
    arr.push(params[i].write());
  }

  const bytes = _util.default.concatUint8Array(arr);

  if (hash_algorithm) {
    const hash = get_hash_fn(hash_algorithm)(bytes);
    return _util.default.concatUint8Array([bytes, hash]);
  }
...
SecretKey.prototype.encrypt = async function (passphrase) {
  ...
  const s2k = new _s2k.default();
  s2k.salt = await _crypto.default.random.getRandomBytes(8);
  ...
  const blockLen = _crypto.default.cipher[symmetric].blockSize;
  const iv = await _crypto.default.random.getRandomBytes(blockLen);
  let arr;

  if (this.version === 5) {
    const aead = 'eax';

    const optionalFields = _util.default.concatUint8Array([new Uint8Array([_enums.default.write(_enums.default.symmetric, symmetric), _enums.default.write(_enums.default.aead, aead)]), s2k.write(), iv]);

    arr = [new Uint8Array([253, optionalFields.length])];
    arr.push(optionalFields);
    const mode = _crypto.default[aead];
    ...
    arr.push(_util.default.writeNumber(encrypted.length, 4));
    arr.push(encrypted);
  } else {
    arr = [new Uint8Array([254, _enums.default.write(_enums.default.symmetric, symmetric)])];
    arr.push(s2k.write());
    arr.push(iv);
    arr.push(_crypto.default.cfb.normalEncrypt(symmetric, key, cleartext, iv));
  }

  this.encrypted = _util.default.concatUint8Array(arr);
  return true;
};

function produceEncryptionKey(s2k, passphrase, algorithm) {
  return s2k.produce_key(passphrase, _crypto.default.cipher[algorithm].keySize);
}
/**
...
SecretKey.prototype.decrypt = async function (passphrase) {
  ...
  let aead;
  let key;
  const s2k_usage = this.encrypted[i++]; // - Only for a version 5 packet, a one-octet scalar octet count of
  ...
  if (s2k_usage === 255 || s2k_usage === 254 || s2k_usage === 253) {
    ...
    symmetric = _enums.default.read(_enums.default.symmetric, symmetric); // - [Optional] If string-to-key usage octet was 253, a one-octet
    //   AEAD algorithm.

    if (s2k_usage === 253) {
      aead = this.encrypted[i++];
      aead = _enums.default.read(_enums.default.aead, aead);
    } // - [Optional] If string-to-key usage octet was 255, 254, or 253, a
    ...
    symmetric = s2k_usage;
    symmetric = _enums.default.read(_enums.default.symmetric, symmetric);
    key = _crypto.default.hash.md5(passphrase);
  } // - [Optional] If secret data is encrypted (string-to-key usage octet
  //   not zero), an Initial Vector (IV) of the same length as the
  //   cipher's block size.


  const iv = this.encrypted.subarray(i, i + _crypto.default.cipher[symmetric].blockSize);
  ...
  if (aead) {
    ...
    cleartext = _crypto.default.cfb.normalDecrypt(symmetric, key, ciphertext, iv);
  }

  const hash = s2k_usage === 253 ? null : s2k_usage === 254 ? 'sha1' : 'mod';
...
SecretKey.prototype.generate = async function (bits, curve) {
  const algo = _enums.default.write(_enums.default.publicKey, this.algorithm);

  this.params = await _crypto.default.generateParams(algo, bits, curve);
  this.isEncrypted = false;
};
/**
...
SecretKey.prototype.clearPrivateParams = function () {
  ...
  this.params = this.params.slice(0, _crypto.default.getPubKeyParamTypes(algo).length);
  this.isEncrypted = true;
};
/**
...
SecretKey.prototype.postCloneTypeFix = function () {
  ...
  const types = [].concat(_crypto.default.getPubKeyParamTypes(algo), _crypto.default.getPrivKeyParamTypes(algo));
  ...
  if (this.keyid) {
    this.keyid = _keyid.default.fromClone(this.keyid);
  }
};

var _default = SecretKey;
