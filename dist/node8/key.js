...
Key.prototype.packetlist2structure = function (packetlist) {
  ...
  for (let i = 0; i < packetlist.length; i++) {
    switch (packetlist[i].tag) {
      case _enums.default.packet.publicKey:
      case _enums.default.packet.secretKey:
        this.keyPacket = packetlist[i];
        primaryKeyId = this.getKeyId();
        break;

      case _enums.default.packet.userid:
      case _enums.default.packet.userAttribute:
        user = new User(packetlist[i]);
        this.users.push(user);
        break;

      case _enums.default.packet.publicSubkey:
      case _enums.default.packet.secretSubkey:
        user = null;
        subKey = new SubKey(packetlist[i]);
        this.subKeys.push(subKey);
        break;

      case _enums.default.packet.signature:
        switch (packetlist[i].signatureType) {
          case _enums.default.signature.cert_generic:
          case _enums.default.signature.cert_persona:
          case _enums.default.signature.cert_casual:
          case _enums.default.signature.cert_positive:
            if (!user) {
              _util.default.print_debug('Dropping certification signatures without preceding user packet');
          ...
          case _enums.default.signature.cert_revocation:
            if (user) {
          ...
          case _enums.default.signature.key:
            this.directSignatures.push(packetlist[i]);
            break;

          case _enums.default.signature.subkey_binding:
            if (!subKey) {
              _util.default.print_debug('Dropping subkey binding signature without preceding subkey packet');
          ...
          case _enums.default.signature.key_revocation:
            this.revocationSignatures.push(packetlist[i]);
            break;

          case _enums.default.signature.subkey_revocation:
            if (!subKey) {
              _util.default.print_debug('Dropping subkey revocation signature without preceding subkey packet');
...
Key.prototype.isPublic = function () {
  return this.keyPacket.tag === _enums.default.packet.publicKey;
};
/**
...
Key.prototype.isPrivate = function () {
  return this.keyPacket.tag === _enums.default.packet.secretKey;
};
/**
  ...
  for (let i = 0; i < keyPackets.length; i++) {
    switch (keyPackets[i].tag) {
      case _enums.default.packet.secretKey:
        ...
        pubKeyPacket = new _packet.default.PublicKey();
      ...
      case _enums.default.packet.secretSubkey:
        bytes = keyPackets[i].writePublicKey();
        pubSubkeyPacket = new _packet.default.PublicSubkey();
...
Key.prototype.armor = function () {
  const type = this.isPublic() ? _enums.default.armor.public_key : _enums.default.armor.private_key;
  return _armor.default.encode(type, this.toPacketlist().write());
};
/**
...
async function getLatestValidSignature(signatures, primaryKey, dataToVerify, date = new Date()) {
  ...
  for (let i = signatures.length - 1; i >= 0; i--) {
    if ((!signature || signatures[i].created >= signature.created) && // check binding signature is not expired (ie, check for V4 expiration time)
    !signatures[i].isExpired(date) && ( // check binding signature is verified
...
Key.prototype.getSigningKey = async function (keyId = null, date = new Date(), userId = {}) {
  ...
  if ((await this.verifyPrimaryKey(date, userId)) === _enums.default.keyStatus.valid) {
    const subKeys = this.subKeys.slice().sort((a, b) => b.keyPacket.created - a.keyPacket.created);

    for (let i = 0; i < subKeys.length; i++) {
      if (!keyId || subKeys[i].getKeyId().equals(keyId)) {
        if ((await subKeys[i].verify(primaryKey, date)) === _enums.default.keyStatus.valid) {
          const dataToVerify = {
            key: primaryKey,
  ...
  function isValidSigningKeyPacket(keyPacket, signature) {
    ...
    return keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.rsa_encrypt) && keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.elgamal) && keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.ecdh) && (!signature.keyFlags || (signature.keyFlags[0] & _enums.default.keyFlags.sign_data) !== 0);
  }
};
/**
...
Key.prototype.getEncryptionKey = async function (keyId, date = new Date(), userId = {}) {
  ...
  function isValidEncryptionKeyPacket(keyPacket, signature) {
    ...
    return keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.dsa) && keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.rsa_sign) && keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.ecdsa) && keyPacket.algorithm !== _enums.default.read(_enums.default.publicKey, _enums.default.publicKey.eddsa) && (!signature.keyFlags || (signature.keyFlags[0] & _enums.default.keyFlags.encrypt_communication) !== 0 || (signature.keyFlags[0] & _enums.default.keyFlags.encrypt_storage) !== 0);
  }
};
/**
...
Key.prototype.encrypt = async function (passphrases, keyId = null) {
  ...
  passphrases = _util.default.isArray(passphrases) ? passphrases : new Array(keys.length).fill(passphrases);
...
Key.prototype.decrypt = async function (passphrases, keyId = null) {
  ...
  passphrases = _util.default.isArray(passphrases) ? passphrases : [passphrases];
...
Key.prototype.isRevoked = async function (signature, key, date = new Date()) {
  return isDataRevoked(this.keyPacket, {
    key: this.keyPacket
...
Key.prototype.verifyPrimaryKey = async function (date = new Date(), userId = {}) {
  const primaryKey = this.keyPacket; // check for key revocation signatures

  if (await this.isRevoked(null, null, date)) {
    return _enums.default.keyStatus.revoked;
  ...
  if (!this.users.some(user => user.userId && user.selfCertifications.length)) {
    return _enums.default.keyStatus.no_self_cert;
  ...
  const {
    user,
    selfCertification
  } = (await this.getPrimaryUser(date, userId)) || {};

  if (!user) {
    return _enums.default.keyStatus.invalid;
  } // check for expiration time


  if (isDataExpired(primaryKey, selfCertification, date)) {
    return _enums.default.keyStatus.expired;
  }

  return _enums.default.keyStatus.valid;
};
/**
...
Key.prototype.update = async function (key) {
  if ((await key.verifyPrimaryKey()) === _enums.default.keyStatus.invalid) {
    return;
  }
  ...
  await mergeSignatures(key, this, 'directSignatures'); // TODO replace when Promise.some or Promise.any are implemented
  // users
    ...
    if (!dest[attr].length) {
      ...
      await Promise.all(source.map(async function (sourceSig) {
        if (!sourceSig.isExpired() && (!checkFn || (await checkFn(sourceSig))) && !dest[attr].some(function (destSig) {
          return _util.default.equalsUint8Array(destSig.signature, sourceSig.signature);
        })) {
...
Key.prototype.revoke = async function ({
  flag: reasonForRevocationFlag = _enums.default.reasonForRevocation.no_reason,
  ...
  key.revocationSignatures.push((await createSignaturePacket(dataToSign, null, this.keyPacket, {
    signatureType: _enums.default.signature.key_revocation,
    reasonForRevocationFlag: _enums.default.write(_enums.default.reasonForRevocation, reasonForRevocationFlag),
...
Key.prototype.getRevocationCertificate = async function () {
  ...
  if (revocationSignature) {
    ...
    return _armor.default.encode(_enums.default.armor.public_key, packetlist.write(), null, null, 'This is a revocation certificate');
  }
};
/**
...
Key.prototype.applyRevocationCertificate = async function (revocationCertificate) {
  const input = await _armor.default.decode(revocationCertificate);
  const packetlist = new _packet.default.List();
  await packetlist.read(input.data);
  const revocationSignature = packetlist.findPacket(_enums.default.packet.signature);

  if (!revocationSignature || revocationSignature.signatureType !== _enums.default.signature.key_revocation) {
  ...
  if (!(await revocationSignature.verify(this.keyPacket, {
    key: this.keyPacket
  }))) {
...
Key.prototype.verifyPrimaryUser = async function (keys) {
  ...
  const results = keys ? await user.verifyAllCertifications(primaryKey, keys) : [{
    ...
    valid: (await user.verify(primaryKey)) === _enums.default.keyStatus.valid
  }];
  return results;
};
/**
...
function User(userPacket) {
  ...
  this.userId = userPacket.tag === _enums.default.packet.userid ? userPacket : null;
  this.userAttribute = userPacket.tag === _enums.default.packet.userAttribute ? userPacket : null;
...
User.prototype.sign = async function (primaryKey, privateKeys) {
  ...
  user.otherCertifications = await Promise.all(privateKeys.map(async function (privateKey) {
    ...
    return createSignaturePacket(dataToSign, privateKey, signingKey.keyPacket, {
      ...
      signatureType: _enums.default.signature.cert_generic,
      keyFlags: [_enums.default.keyFlags.certify_keys | _enums.default.keyFlags.sign_data]
    });
  }));
...
async function createSignaturePacket(dataToSign, privateKey, signingKeyPacket, signatureProperties, date, userId) {
  ...
  const signaturePacket = new _packet.default.Signature(date);
...
User.prototype.verifyAllCertifications = async function (primaryKey, keys) {
  ...
  return Promise.all(certifications.map(async function (certification) {
    ...
    return {
      ...
      valid: status === undefined ? null : status === _enums.default.keyStatus.valid
    };
  }));
};
/**
...
User.prototype.verify = async function (primaryKey) {
  ...
  const results = [_enums.default.keyStatus.invalid].concat((await Promise.all(this.selfCertifications.map(async function (selfCertification) {
  ...
  return results.some(status => status === _enums.default.keyStatus.valid) ? _enums.default.keyStatus.valid : results.pop();
};
/**
...
User.prototype.update = async function (user, primaryKey) {
  ...
  await mergeSignatures(user, this, 'otherCertifications'); // revocation signatures
...
SubKey.prototype.verify = async function (primaryKey, date = new Date()) {
  ...
  const bindingSignature = await getLatestValidSignature(this.bindingSignatures, primaryKey, dataToVerify, date); // check binding signature is verified
  ...
  return _enums.default.keyStatus.valid; // binding signature passed all checks
};
/**
...
SubKey.prototype.update = async function (subKey, primaryKey) {
  if ((await subKey.verify(primaryKey)) === _enums.default.keyStatus.invalid) {
    return;
  }
  ...
  if (this.keyPacket.tag === _enums.default.packet.publicSubkey && subKey.keyPacket.tag === _enums.default.packet.secretSubkey) {
...
SubKey.prototype.revoke = async function (primaryKey, {
  ...
  subKey.revocationSignatures.push((await createSignaturePacket(dataToSign, null, primaryKey, {
    signatureType: _enums.default.signature.subkey_revocation,
    reasonForRevocationFlag: _enums.default.write(_enums.default.reasonForRevocation, reasonForRevocationFlag),
    reasonForRevocationString
  }, date)));
...
async function read(data) {
  ...
  try {
    ...
    if (packetlist.filterByTag(_enums.default.packet.signature).some(signature => signature.revocationKeyClass !== null)) {
    ...
    const keyIndex = packetlist.indexOfTag(_enums.default.packet.publicKey, _enums.default.packet.secretKey);
...
async function readArmored(armoredText) {
  try {
    ...
    if (!(input.type === _enums.default.armor.public_key || input.type === _enums.default.armor.private_key)) {
...
async function generate(options) {
  ...
  function sanitizeKeyOptions(options, subkeyDefaults = {}) {
    ...
    options.passphrase = _util.default.isString(options.passphrase) ? options.passphrase : subkeyDefaults.passphrase;
    options.date = options.date || subkeyDefaults.date;
    options.sign = options.sign || false;

    if (options.curve) {
      try {
        options.curve = _enums.default.write(_enums.default.curve, options.curve);
      } catch (e) {
        throw new Error('Not valid curve.');
      }

      if (options.curve === _enums.default.curve.ed25519 || options.curve === _enums.default.curve.curve25519) {
        if (options.sign) {
          options.algorithm = _enums.default.publicKey.eddsa;
          options.curve = _enums.default.curve.ed25519;
        } else {
          options.algorithm = _enums.default.publicKey.ecdh;
          options.curve = _enums.default.curve.curve25519;
        }
      } else {
        if (options.sign) {
          options.algorithm = _enums.default.publicKey.ecdsa;
        } else {
          options.algorithm = _enums.default.publicKey.ecdh;
        }
      }
    } else if (options.numBits) {
      options.algorithm = _enums.default.publicKey.rsa_encrypt_sign;
    } else {
  ...
  async function generateSecretKey(options) {
    const secretKeyPacket = new _packet.default.SecretKey(options.date);
    secretKeyPacket.packets = null;
    secretKeyPacket.algorithm = _enums.default.read(_enums.default.publicKey, options.algorithm);
    await secretKeyPacket.generate(options.numBits, options.curve);
    return secretKeyPacket;
  }

  async function generateSecretSubkey(options) {
    const secretSubkeyPacket = new _packet.default.SecretSubkey(options.date);
    secretSubkeyPacket.packets = null;
    secretSubkeyPacket.algorithm = _enums.default.read(_enums.default.publicKey, options.algorithm);
...
async function reformat(options) {
  ...
  for (let i = 0; i < packetlist.length; i++) {
    if (packetlist[i].tag === _enums.default.packet.secretKey) {
      secretKeyPacket = packetlist[i];
    } else if (packetlist[i].tag === _enums.default.packet.secretSubkey) {
...
async function wrapKeyObject(secretKeyPacket, secretSubkeyPackets, options) {
  ...
  await Promise.all(options.userIds.map(async function (userId, index) {
    const userIdPacket = new _packet.default.Userid();
    userIdPacket.format(userId);
    const dataToSign = {};
    dataToSign.userId = userIdPacket;
    dataToSign.key = secretKeyPacket;
    const signaturePacket = new _packet.default.Signature(options.date);
    signaturePacket.signatureType = _enums.default.signature.cert_generic;
    signaturePacket.publicKeyAlgorithm = secretKeyPacket.algorithm;
    signaturePacket.hashAlgorithm = await getPreferredHashAlgo(null, secretKeyPacket);
    signaturePacket.keyFlags = [_enums.default.keyFlags.certify_keys | _enums.default.keyFlags.sign_data];
    signaturePacket.preferredSymmetricAlgorithms = []; // prefer aes256, aes128, then aes192 (no WebCrypto support: https://www.chromium.org/blink/webcrypto#TOC-AES-support)

    signaturePacket.preferredSymmetricAlgorithms.push(_enums.default.symmetric.aes256);
    signaturePacket.preferredSymmetricAlgorithms.push(_enums.default.symmetric.aes128);
    signaturePacket.preferredSymmetricAlgorithms.push(_enums.default.symmetric.aes192);
    signaturePacket.preferredSymmetricAlgorithms.push(_enums.default.symmetric.cast5);
    signaturePacket.preferredSymmetricAlgorithms.push(_enums.default.symmetric.tripledes);

    if (_config.default.aead_protect && _config.default.aead_protect_version === 4) {
      signaturePacket.preferredAeadAlgorithms = [];
      signaturePacket.preferredAeadAlgorithms.push(_enums.default.aead.eax);
      signaturePacket.preferredAeadAlgorithms.push(_enums.default.aead.ocb);
    }

    signaturePacket.preferredHashAlgorithms = []; // prefer fast asm.js implementations (SHA-256). SHA-1 will not be secure much longer...move to bottom of list

    signaturePacket.preferredHashAlgorithms.push(_enums.default.hash.sha256);
    signaturePacket.preferredHashAlgorithms.push(_enums.default.hash.sha512);
    signaturePacket.preferredHashAlgorithms.push(_enums.default.hash.sha1);
    signaturePacket.preferredCompressionAlgorithms = [];
    signaturePacket.preferredCompressionAlgorithms.push(_enums.default.compression.zlib);
    signaturePacket.preferredCompressionAlgorithms.push(_enums.default.compression.zip);

    if (index === 0) {
      signaturePacket.isPrimaryUserID = true;
    }

    if (_config.default.integrity_protect) {
      signaturePacket.features = [0];
      signaturePacket.features[0] |= _enums.default.features.modification_detection;
    }

    if (_config.default.aead_protect && _config.default.aead_protect_version === 4) {
      signaturePacket.features || (signaturePacket.features = [0]);
      signaturePacket.features[0] |= _enums.default.features.aead;
      signaturePacket.features[0] |= _enums.default.features.v5_keys;
    }
  ...
  await Promise.all(secretSubkeyPackets.map(async function (secretSubkeyPacket, index) {
    ...
    const subkeySignaturePacket = new _packet.default.Signature(subkeyOptions.date);
    subkeySignaturePacket.signatureType = _enums.default.signature.subkey_binding;
    ...
    subkeySignaturePacket.keyFlags = subkeyOptions.sign ? _enums.default.keyFlags.sign_data : [_enums.default.keyFlags.encrypt_communication | _enums.default.keyFlags.encrypt_storage];
  ...
  packetlist.push((await createSignaturePacket(dataToSign, null, secretKeyPacket, {
    signatureType: _enums.default.signature.key_revocation,
    reasonForRevocationFlag: _enums.default.reasonForRevocation.no_reason,
    reasonForRevocationString: ''
  }, options.date))); // set passphrase protection
...
async function isDataRevoked(primaryKey, dataToVerify, revocations, signature, key, date = new Date()) {
  ...
  const normDate = _util.default.normalizeDate(date);

  const revocationKeyIds = [];
  await Promise.all(revocations.map(async function (revocationSignature) {
    if ( // Note: a third-party revocation signature could legitimately revoke a
    ...
    (!signature || revocationSignature.issuerKeyId.equals(signature.issuerKeyId)) && !(_config.default.revocations_expire && revocationSignature.isExpired(normDate)) && (revocationSignature.verified || (await revocationSignature.verify(key, dataToVerify)))) {
...
function getExpirationTime(keyPacket, signature) {
  let expirationTime; // check V4 expiration time
...
async function getPreferredHashAlgo(key, keyPacket, date = new Date(), userId = {}) {
  let hash_algo = _config.default.prefer_hash_algorithm;
  ...
  if (key instanceof Key) {
    ...
    if (primaryUser && primaryUser.selfCertification.preferredHashAlgorithms) {
      ...
      hash_algo = _crypto.default.hash.getHashByteLength(hash_algo) <= _crypto.default.hash.getHashByteLength(pref_algo) ? pref_algo : hash_algo;
    }
  }

  switch (Object.getPrototypeOf(keyPacket)) {
    case _packet.default.SecretKey.prototype:
    case _packet.default.PublicKey.prototype:
    case _packet.default.SecretSubkey.prototype:
    case _packet.default.PublicSubkey.prototype:
      switch (keyPacket.algorithm) {
        case 'ecdh':
        case 'ecdsa':
        case 'eddsa':
          pref_algo = _crypto.default.publicKey.elliptic.getPreferredHashAlgo(keyPacket.params[0]);
      }

  }

  return _crypto.default.hash.getHashByteLength(hash_algo) <= _crypto.default.hash.getHashByteLength(pref_algo) ? pref_algo : hash_algo;
}
/**
...
async function getPreferredAlgo(type, keys, date = new Date(), userId = {}) {
  ...
  const defaultAlgo = type === 'symmetric' ? _config.default.encryption_cipher : _config.default.aead_mode;
  const prioMap = {};
  ...
  Object.values(prioMap).forEach(({
    ...
    try {
      if (algo !== _enums.default[type].plaintext && algo !== _enums.default[type].idea && // not implemented
      _enums.default.read(_enums.default[type], algo) && // known algorithm
...
async function isAeadSupported(keys, date = new Date(), userId = {}) {
  let supported = true; // TODO replace when Promise.some or Promise.any are implemented

  await Promise.all(keys.map(async function (key) {
    const primaryUser = await key.getPrimaryUser(date, userId);

    if (!primaryUser || !primaryUser.selfCertification.features || !(primaryUser.selfCertification.features[0] & _enums.default.features.aead)) {
      supported = false;
    }
  }));
  return supported;
