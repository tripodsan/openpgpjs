...
function LocalStore(prefix) {
  ...
  if (typeof window !== 'undefined' && window.localStorage) {
    ...
    this.storage = new (require('node-localstorage').LocalStorage)(_config.default.node_store);
  }
}
/*
  ...
  if (armoredKeys !== null && armoredKeys.length !== 0) {
    ...
    for (let i = 0; i < armoredKeys.length; i++) {
      ...
      if (!key.err) {
        ...
        _util.default.print_debug("Error reading armored key from keyring index: " + i);
      }
    }
  }
...
async function storeKeys(storage, itemname, keys) {
  if (keys.length) {
    const armoredKeys = await Promise.all(keys.map(key => _webStreamTools.default.readToEnd(key.armor())));
