...
const nodeCrypto = _util.default.detectNode() && require('crypto');
...
var _default = {
  ...
  getRandomBN: async function (min, max) {
    ...
    const bytes = modulus.byteLength(); // Using a while loop is necessary to avoid bias introduced by the mod operation.
    // However, we request 64 extra random bits so that the bias is negligible.
    // Section B.1.1 here: https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf

    const r = new _bn.default((await this.getRandomBytes(bytes + 8)));
