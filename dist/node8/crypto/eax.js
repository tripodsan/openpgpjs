...
var _ctr = require("@tripod/asmcrypto.js/dist_compat/aes/ctr");

var _cmac = _interopRequireDefault(require("./cmac"));
...
one[blockLength - 1] = 1;
const two = new Uint8Array(blockLength);
two[blockLength - 1] = 2;

async function OMAC(key) {
  const cmac = await (0, _cmac.default)(key);
  return function (t, message) {
    return cmac(_util.default.concatUint8Array([t, message]));
  };
}

async function CTR(key) {
  if (_util.default.getWebCrypto() && key.length !== 24 && // WebCrypto (no 192 bit support) see: https://www.chromium.org/blink/webcrypto#TOC-AES-support
  navigator.userAgent.indexOf('Edge') === -1) {
    key = await webCrypto.importKey('raw', key, {
      name: 'AES-CTR',
      length: key.length * 8
    }, false, ['encrypt']);
  ...
  if (_util.default.getNodeCrypto()) {
    // Node crypto library
    key = new Buffer(key);
    return async function (pt, iv) {
      pt = new Buffer(pt);
...
async function EAX(cipher, key) {
  ...
  return {
    ...
    encrypt: async function (plaintext, nonce, adata) {
      ...
      return _util.default.concatUint8Array([ciphered, tag]);
    },
    ...
    decrypt: async function (ciphertext, nonce, adata) {
      ...
      if (!_util.default.equalsUint8Array(ctTag, tag)) throw new Error('Authentication tag mismatch');
