...
var _prime = _interopRequireDefault(require("./prime"));
...
var _default = {
  ...
  sign: async function (m, n, e, d) {
    ...
    const nred = new _bn.default.red(n);
  ...
  decrypt: async function (m, n, e, d, p, q, u) {
    ...
    const pred = new _bn.default.red(p);
    const qred = new _bn.default.red(q);
    const nred = new _bn.default.red(n);
    let blinder;
    let unblinder;

    if (_config.default.rsa_blinding) {
      unblinder = (await _random.default.getRandomBN(new _bn.default(2), n)).toRed(nred);
  ...
  generate: async function (B, E) {
    let key;
    E = new _bn.default(E, 16);

    const webCrypto = _util.default.getWebCryptoAll(); // Native RSA keygen using Web Crypto
    ...
    if (webCrypto) {
      ...
      jwk = await promisifyIE11Op(jwk, 'Error exporting RSA key pair.'); // parse raw ArrayBuffer bytes to jwk/json (WebKit/Safari/IE11 quirk)
      ...
      key = {};
      key.n = new _bn.default(_util.default.b64_to_Uint8Array(jwk.n));
      key.e = E;
      key.d = new _bn.default(_util.default.b64_to_Uint8Array(jwk.d));
      key.p = new _bn.default(_util.default.b64_to_Uint8Array(jwk.p));
      key.q = new _bn.default(_util.default.b64_to_Uint8Array(jwk.q));
    ...
    let p = await _prime.default.randomProbablePrime(B - (B >> 1), E, 40);
    let q = await _prime.default.randomProbablePrime(B >> 1, E, 40);
