...
var _default = {
  ...
  sign: async function (hash_algo, hashed, g, p, q, x) {
    ...
    let s;
    let t;
    const redp = new _bn.default.red(p);
    const redq = new _bn.default.red(q);
    const gred = g.toRed(redp);
    const xred = x.toRed(redq); // If the output size of the chosen hash is larger than the number of
    // bits of q, the hash result is truncated to fit by taking the number
    // of leftmost bits equal to the number of bits of q.  This (possibly
    // truncated) hash function result is treated as a number and used
    // directly in the DSA signature algorithm.

    const h = new _bn.default(_util.default.getLeftNBits(hashed, q.bitLength())).toRed(redq); // FIPS-186-4, section 4.6:
    ...
    while (true) {
      ...
      k = await _random.default.getRandomBN(one, q); // returns in [1, q-1]
  ...
  verify: async function (hash_algo, r, s, hashed, g, p, q, y) {
    if (zero.ucmp(r) >= 0 || r.ucmp(q) >= 0 || zero.ucmp(s) >= 0 || s.ucmp(q) >= 0) {
      _util.default.print_debug("invalid DSA Signature");
    ...
    const redp = new _bn.default.red(p);
    const redq = new _bn.default.red(q);
    const h = new _bn.default(_util.default.getLeftNBits(hashed, q.bitLength()));
