...
const ivLength = 15; // https://tools.ietf.org/html/draft-ietf-openpgp-rfc4880bis-04#section-5.16.2:
...
async function OCB(cipher, key) {
  ...
  function constructKeyVariables(cipher, key) {
    const aes = new _cipher.default[cipher](key);
    encipher = aes.encrypt.bind(aes);
    decipher = aes.decrypt.bind(aes);
    const mask_x = encipher(zeroBlock);

    const mask_$ = _util.default.double(mask_x);

    mask = [];
    mask[0] = _util.default.double(mask_$);
    mask.x = mask_x;
    mask.$ = mask_$;
  }

  function extendKeyVariables(text, adata) {
    const newMaxNtz = _util.default.nbits(Math.max(text.length, adata.length) / blockLength | 0) - 1;

    for (let i = maxNtz + 1; i <= newMaxNtz; i++) {
      mask[i] = _util.default.double(mask[i - 1]);
    }
  ...
  function crypt(fn, text, nonce, adata) {
    ...
    const paddedNonce = _util.default.concatUint8Array([zeroBlock.subarray(0, ivLength - nonce.length), one, nonce]); //    bottom = str2num(Nonce[123..128])


    const bottom = paddedNonce[blockLength - 1] & 0b111111; //    Ktop = ENCIPHER(K, Nonce[1..122] || zeros(6))

    paddedNonce[blockLength - 1] &= 0b11000000;
    const kTop = encipher(paddedNonce); //    Stretch = Ktop || (Ktop[1..64] xor Ktop[9..72])

    const stretched = _util.default.concatUint8Array([kTop, xor(kTop.subarray(0, 8), kTop.subarray(1, 9))]); //    Offset_0 = Stretch[1+bottom..128+bottom]


    const offset = _util.default.shiftRight(stretched.subarray(0 + (bottom >> 3), 17 + (bottom >> 3)), 8 - (bottom & 7)).subarray(1); //    Checksum_0 = zeros(128)
  ...
  return {
    ...
    decrypt: async function (ciphertext, nonce, adata) {
      ...
      if (_util.default.equalsUint8Array(tag, crypted.subarray(-tagLength))) {
