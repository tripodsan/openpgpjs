...
var _default = {
  ...
  encrypt: function (prefixrandom, cipherfn, plaintext, key, resync) {
    cipherfn = new _cipher.default[cipherfn](key);
    ...
    let n;
    let begin;
    const offset = resync ? 0 : 2; // 1.  The feedback register (FR) is set to the IV, which is all zeros.
    ...
    FRE = cipherfn.encrypt(FR); // 3.  FRE is xored with the first BS octets of random data prefixed to
    ...
    FR.set(ciphertext.subarray(0, block_size)); // 5.  FR is encrypted to produce FRE, the encryption of the first BS
    //     octets of ciphertext.

    FRE = cipherfn.encrypt(FR); // 6.  The left two octets of FRE get xored with the next two octets of
    ...
    FRE = cipherfn.encrypt(FR); // 9.  FRE is xored with the first BS octets of the given plaintext, now
    ...
    for (n = block_size; n < plaintext.length + offset; n += block_size) {
      ...
      // an 8-octet block).
      begin = n + 2 - offset;
      FR.set(ciphertext.subarray(begin, begin + block_size)); // 11. FR is encrypted to produce FRE.

      FRE = cipherfn.encrypt(FR); // 12. FRE is xored with the next BS octets of plaintext, to produce
  ...
  decrypt: function (cipherfn, key, ciphertext, resync) {
    ...
    let j;
    let n;
    let text = new Uint8Array(ciphertext.length - block_size); // initialisation vector
    ...
    ablock = cipherfn.encrypt(ablock); // test check octets
