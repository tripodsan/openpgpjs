...
async function getPkcs1Padding(length) {
  ...
  while (result.length < length) {
    const randomBytes = await _random.default.getRandomBytes(length - result.length);
...
eme.encode = async function (M, k) {
  const mLen = M.length; // length checking
  ...
  const PS = await getPkcs1Padding(k - mLen - 3); // Concatenate PS, the message M, and other padding to form an
...
emsa.encode = async function (algo, hashed, emLen) {
  ...
  const H = _util.default.Uint8Array_to_str(hashed);

  if (H.length !== _hash.default.getHashByteLength(algo)) {
  ...
  return _util.default.str_to_hex(EM);
};
